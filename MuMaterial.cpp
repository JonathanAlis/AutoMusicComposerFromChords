...
MuMaterial::MuMaterial( const MuMaterial & inMaterial, int fromVoice )
{
    ...
    if(inMaterial.voices != NULL)
    {
        if(fromVoice < inMaterial.numOfVoices)
        {
...
void MuMaterial::SetVoice(int voiceNum, MuMaterial inMaterial, int inVoice)
{
    lastError.Set(MuERROR_NONE);
	long i, n;
	MuNote theNote;
	
	// if calling code requests a voice number which does not exist in inMaterial...
	// .. we bail out.
	if( !( ( inVoice >= 0 ) && ( inVoice < inMaterial.NumberOfVoices() )) )
	{
		lastError.Set(MuERROR_INVALID_VOICE_NUMBER);
		return;
	}
	// if calling code asks for negative voice number, ...
	// we bail out.
	if( voiceNum < 0 )
	{
		lastError.Set(MuERROR_INVALID_VOICE_NUMBER);
	...
	n = inMaterial.NumberOfNotes(inVoice);
	for(i = 0; i < n; i++)
	{
		theNote = inMaterial.GetNote( inVoice, i );
		if( (inMaterial.LastError()).Get() == MuERROR_NONE )
			lastError.Set( voices[voiceNum].AddNote( theNote ) );
		else
		{	
			lastError.Set( inMaterial.LastError() );
			return;
		}
	}
...
void MuMaterial::Mix( MuMaterial inMaterial)
{
	lastError.Set(MuERROR_NONE);
	MuError err(MuERROR_NONE);
	MuNote note;
...
void MuMaterial::Mix(int voiceNumber,  MuMaterial  inMaterial, int inVoice)
{
	lastError.Set(MuERROR_NONE);
	MuError err(MuERROR_NONE);
	MuVoice sourceVoice;
	...
	for(i = 0; i < n; i++)
	{
		note = inMaterial.GetNote(inVoice, i);
		err = inMaterial.LastError();
		if(err.Get() == MuERROR_NONE)
...
void MuMaterial::Append(int voiceNumber, MuMaterial inMaterial, int inVoice)
{
	lastError.Set(MuERROR_NONE);
	long i, n;
	float end;
	...
	// if calling code asks for negative voice number, we bail out.
	if( voiceNumber < 0 )
	{
		lastError.Set(MuERROR_INVALID_VOICE_NUMBER);
		return;
...
void MuMaterial::CyclePitch(int voiceNumber, int times)
{
	...
	int n = NumberOfNotes( voiceNumber );
	int i;
	int tempPitch;
	MuNote note1, note2;
...
{
    lastError.Set(MuERROR_NONE);
	short scalePattern[5] = {0, 2, 3, 7, 9};
	int i;
	float nextStart = 0;
