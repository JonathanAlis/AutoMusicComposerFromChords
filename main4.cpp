...
nota.SetInstr(1);
	...
	while(melodia.GetVoice(0).Dur()<melodia.Dur())
	{//nota1
	if(nota.Start()>2.0*h && nota.Start()<h*3.0 )climax=1;
	else climax=0;
	nota.SetInstr(3);
	nota.SetStart(nota.Start()+nota.Dur());
	if(!climax) if ((base+bet-nota.Pitch())> 6) base-=12;
	if (nota.Pitch()-(base+bet)>= 6) base+=12;
	if (base<30) base+=12;
	if (base>90) base-=12;
	PertenceAoAcorde=0;
	for (int i=12;i<120;i+=12)
	if(melodia.GetNotesSoundingAt(1,melodia.GetVoice(0).Dur()).Contains(1,base+bet))
	PertenceAoAcorde=1;
	
	nota.SetDur(2*(1.0+PertenceAoAcorde)/((1+climax)*2*Between(1,2)));
	nota.SetPitch(base+bet);
	//nota2
	nota2.SetInstr(3);
	nota2.SetStart(nota2.Start()+nota2.Dur());
	if ((base2+bet2-nota2.Pitch())> 6) base2-=12;
	if (nota2.Pitch()-(base2+bet2)>= 6) base2+=12;
	if (base2<30) base2+=12;
	if (base2>110) base2-=12;
	PertenceAoAcorde=0;
	for (int i=12;i<120;i+=12)
	if(melodia.GetNotesSoundingAt(1,melodia.GetVoice(0).Dur()).Contains(1,base2+bet2))
	PertenceAoAcorde=1;
	
	nota2.SetDur((1.0+PertenceAoAcorde)/2*Between(1,2));
	nota2.SetPitch(base2+bet2);
//imprime na tela as notas
cout<<nota.Pitch()<<" ";
//poe as notas no material.
	melodia.AddNote(nota);
//comentar a linha abaixo pra desabilitar a segunda voz.
...
MuMaterial temas=melodia.GetVoice(0).Segments(h);
MuMaterial semifinal=melodia.GetVoice(1);
semifinal.AddVoices(1);
for(int i=0;i<h;i++)
{
int j=0;
semifinal.Append(1,temas,i);
i%(h/n);
//semifinal.
...
melodia.Fit(melodia.Dur()*60.0/bpm);
	...
	semifinal.Score("semifinal.sco");
	melodia.Orchestra("melodia.orc");
	
	system("csound melodia.orc melodia.sco -W -o teste1.wav");
	system("csound melodia.orc semifinal.sco -W -o teste2.wav");
 return 0;
}
