...
if(z!=h)
	...
	{
	if(nota.Start()>(Between(1,2)+0.5)*h && nota.Start()<h*(1.5+Between(1,2)))rapido=1;
	else rapido=0;
	...
	if (base<45+12*rapido) base+=12;
	if (base>95+12*rapido) base-=12;
	PertenceAoAcorde=0;
	...
	{escolha=Between(1,4);temporizador=0;}
	//if (escolha==4) temporizador=2;
	switch(escolha)
	{
	...
	case 4:	nota.SetDur(Between(2,2)*dur4(PertenceAoAcorde,rapido, melodia.GetVoice(0).Dur()));break;
	}
	nota.SetPitch(base+pitch);
	...
	for(int i=0;i<=6;i++)
		...
		for(int j=0;j<PrimAc.NumberOfNotes();j++)	
			if(abs(melodia.GetNote(0,melodia.NumberOfNotes(0)-1).Pitch()-PrimAc.GetNote(j).Pitch())<minimo)
				...
				ultima.SetPitch(PrimAc.GetNote(j).Pitch());

				}

		}
	melodia.AddNote(ultima);
...
melodia.Fit(melodia.Dur()*60.0/bpm);
	...
	system("csound orquestra.orc melodia.sco -W -o teste1.wav");
